...
def main():
    ...
    log.info("A browser window should open automatically, if not enter the URL above")
    ...
    log.debug(f"Fetching periods from: {periods_url}")
    periods = requests.get(periods_url, headers=headers)
    with open(f"periods.json", "w") as f:
        json.dump(periods.json(), f, ensure_ascii=False, indent=4)
    ...
    for p in data:
        ...
        log.debug(f"Fetching consumption data from: {consumption_url}")
        consumption = requests.get(consumption_url, headers=headers)
        with open(f"{period}.json", "w") as f:
            json.dump(consumption.json(), f, ensure_ascii=False, indent=4)
        log.debug(f"Response body:\n{consumption.text}")
        ...
        log.debug(f"Fetching average consumption data from {consumption_average_url}")
        consumption_average = requests.get(consumption_average_url, headers=headers)
        with open(f"{period}-average.json", "w") as f:
            json.dump(consumption_average.json(), f, ensure_ascii=False, indent=4)
